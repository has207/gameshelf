...
class GameShelfWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, app, controller):
        ...
        if hasattr(self, 'sidebar_listview') and self.sidebar_listview is not None:
            ...
            self.refresh_sidebar_runners()

    def refresh_sidebar_runners(self):
        """Refresh the sidebar list with current runners"""
        if not hasattr(self, 'sidebar_store') or not self.sidebar_listview:
            return
        ...
        current_selection = -1
        if selection_model:
            current_selection = selection_model.get_selected()
        ...
        # Setup factory and model if first time
        if not selection_model:
            ...
            # Select the first item (Games)
            selection.set_selected(0)
        else:
            # Try to restore previous selection
            if current_selection >= 0 and current_selection < self.sidebar_store.get_n_items():
                selection_model.set_selected(current_selection)
            else:
...
@Gtk.Template(filename=os.path.join(os.path.dirname(__file__), "layout", "runner_dialog.ui"))
class RunnerDialog(Adw.Window):
    ...
    def validate_form(self, *args):
        ...
        if self.edit_mode and self.runner:
            ...
            # In add mode, just check for title and command
...
@Gtk.Template(filename=os.path.join(os.path.dirname(__file__), "layout", "runner_list_row.ui"))
...
@Gtk.Template(filename=os.path.join(os.path.dirname(__file__), "layout", "runners_manager.ui"))
...
class GameShelfController:
    ...
    def reload_data(self):
        ...
        # Refresh the sidebar if window has a refresh_sidebar_runners method
        if self.window and hasattr(self.window, 'refresh_sidebar_runners'):
