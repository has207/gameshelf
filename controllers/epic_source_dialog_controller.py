...
@Gtk.Template(filename="layout/epic_source_dialog.ui")
class EpicSourceDialog(Gtk.Dialog):
    __gtype_name__ = "EpicSourceDialog"

    name_entry = Gtk.Template.Child()
    active_switch = Gtk.Template.Child()
    download_images_switch = Gtk.Template.Child()
    optimize_switch = Gtk.Template.Child()
    ...
    def __init__(self, parent=None, source=None, source_handler=None, **kwargs):
        ...
        if self.editing:
            ...
            # Set optimize_switch based on source config
            if source.config and "optimize_catalog" in source.config:
                self.optimize_switch.set_active(source.config.get("optimize_catalog", True))
            else:
                # Default to True if not specified
                self.optimize_switch.set_active(True)

            # For Epic sources, we need to check if there are valid tokens in the tokens directory
            ...
            self.set_title("Add Epic Games Source")
            self.name_entry.set_text("Epic Games")
            self.auth_status_label.set_text("Authentication: Not connected")
        ...
        if not self.editing and self.source and self.source_handler:
            try:
                ...
                if tokens_dir.exists():
                    ...
                    logger.debug(f"Cleaned up token directory for canceled Epic source: {tokens_dir}")
    ...
    def on_auth_clicked(self, button):
        """Handle OAuth authentication with Epic Games"""
        ...
        epic_client = EpicLibraryClient(token_dir=tokens_dir)
        if epic_client.authenticate():
            self.auth_status_label.set_text("Authentication: Connected")
            self.auth_token = "epic_authenticated"  # Just a marker value
        else:
    ...
    def on_dialog_response(self, dialog, response_id):
        ...
        if self.editing:
            ...
            self.source.config["optimize_catalog"] = self.optimize_switch.get_active()
            ...
            self.source.config = {
                ...
                "optimize_catalog": self.optimize_switch.get_active()
            }
    ...
    def _check_auth_status(self, quiet=False):
        """
        Check if the Epic Games authentication is valid
        ...
        # Create an Epic Library instance with this token directory
        epic_client = EpicLibraryClient(token_dir=tokens_dir)

        # Check if the tokens are valid
        is_authenticated = epic_client.is_authenticated()

        logger.debug(f"Epic Games authentication check: {is_authenticated}")
    ...
    def show_dialog(cls, source=None, source_handler=None, parent=None, callback=None):
        """
        Show the Epic Games source dialog
...
GObject.type_register(EpicSourceDialog)
GObject.signal_new("source-saved", EpicSourceDialog,
